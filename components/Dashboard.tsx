...
import { 
  ...
  Database, Wallet, Calendar, FileCheck, ShieldAlert, ArrowUpRight, TrendingUp, Activity, Award, Globe, Lock, FileText, Download, Target, Fingerprint, Mail, RefreshCw, Layers, Heart, Glasses
} from 'lucide-react';
  ...
  return (
    <section className="container mx-auto px-6 max-w-6xl py-12 text-left">
      <motion.div variants={containerVariants} initial="hidden" animate="visible" className="bg-slate-900/60 p-10 md:p-20 rounded-[100px] border border-cyan-500/20 backdrop-blur-3xl shadow-2xl relative overflow-hidden">
        ...
        <div className="mt-16 pt-8 border-t border-white/5 flex justify-between items-center text-[8px] font-black uppercase text-slate-700 tracking-[0.5em] italic"><span>Auditado por B2Y Group Systems & Governance Belém/PA</span><span className="flex items-center gap-2"><Lock size={10}/> Conexão Segura SSL v7.0</span></div>
      </motion.div>
    </section>
  );
}