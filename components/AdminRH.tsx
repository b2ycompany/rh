...
import { 
  ...
  TrendingUp, Heart, Baby, Check, Ban, Zap, Info, Map,
  Users2
} from 'lucide-react';
...
export default function AdminRH({ tickets, vacancies, onAddJob, onUpdateJob, onUpdateERP }: AdminProps) {
  ...
  return (
    <section className="container mx-auto px-6 py-12 text-left">
      <div className="bg-slate-900 p-12 md:p-20 rounded-[80px] border border-white/5 shadow-2xl relative overflow-hidden">
        ...
        {/* OUTRAS TABS (JOBS E BALANCE) MANTIDAS */}
                ...
                <tbody>{tickets.filter(t => t.role === 'client' && t.status === 'Pendente').map(v => (
                  <tr key={v.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                    ...
                    <td className="py-10 flex gap-4">
                      <button onClick={() => onUpdateERP(v.id, { status: 'Aprovado' })} className="p-3 bg-green-500/10 text-green-500 rounded-xl hover:bg-green-500 hover:text-white transition-all"><Check size={18}/></button>
                      <button onClick={() => onUpdateERP(v.id, { status: 'Reprovado' })} className="p-3 bg-red-500/10 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all"><Ban size={18}/></button>
                    </td>
                  </tr>
                ))}</tbody>
             </table>
        ...
        {viewReport && (
          <div className="fixed inset-0 z-[180] bg-slate-950/98 backdrop-blur-3xl flex items-center justify-center p-6 text-left overflow-y-auto py-24">
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-slate-900 border border-cyan-500/30 p-12 md:p-20 rounded-[80px] max-w-6xl w-full relative shadow-2xl">
              ...
              <div className="grid md:grid-cols-3 gap-8 mb-16">
                 ...
                 <div className="p-10 bg-slate-950/50 rounded-[60px] border border-white/5 space-y-6">
                    ...
                    <div className="space-y-4">
                       <div className="flex justify-between border-b border-white/5 pb-2"><span className="text-slate-500 text-[10px] uppercase font-bold">Estado Civil:</span> <span className="text-white text-[12px] font-black italic">{viewReport.marital_status || "Não informado"}</span></div>
                       <div className="flex justify-between border-b border-white/5 pb-2"><span className="text-slate-500 text-[10px] uppercase font-bold">Filhos:</span> <span className="text-white text-[12px] font-black italic">{viewReport.has_children === "Sim" ? `${viewReport.num_children} dependente(s)` : "Nenhum"}</span></div>
                       <div className="flex justify-between border-b border-white/5 pb-2"><span className="text-slate-500 text-[10px] uppercase font-bold">Hobby:</span> <span className="text-white text-[12px] font-black italic uppercase tracking-tighter">{viewReport.hobbies || "Não informado"}</span></div>
                    </div>
                 </div>
                 ...
                 {/* DISC */}
                 <div className="p-10 bg-slate-950/50 rounded-[60px] border border-white/5 space-y-6">
                    <p className="text-[10px] font-black text-purple-500 uppercase tracking-widest flex items-center gap-3"><Target size={16}/> Comportamental DISC</p>
                    <p className="text-3xl text-white font-black italic tracking-tighter uppercase">{viewReport.behavioral_profile || "Auditando"}</p>
                    <div className="w-full bg-white/5 h-2 rounded-full overflow-hidden"><motion.div initial={{width: 0}} animate={{width: '75%'}} className="bg-purple-500 h-full shadow-glow" /></div>
                 </div>
              </div>

              {/* TRAJETÓRIA PROFISSIONAL */}
              <div className="bg-slate-950 p-12 rounded-[60px] border border-white/5 mb-16 relative overflow-hidden">
                <div className="absolute top-0 right-0 p-8 opacity-5 text-cyan-500"><Zap size={100} /></div>
                <p className="text-[12px] font-black uppercase text-slate-700 tracking-[0.5em] italic flex items-center gap-6 border-b border-white/5 pb-6 mb-8"><FileText size={20}/> Resumo de Trajetória Profissional</p>
                <p className="text-xl text-slate-400 leading-loose italic font-light">{viewReport.experience_bio || "Nenhuma bio de trajetória enviada pelo candidato."}</p>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-6 mb-16">
                 {[{k: 'ocean_openness', l: 'Abertura'}, {k: 'ocean_conscientiousness', l: 'Foco'}, {k: 'ocean_extraversion', l: 'Social'}, {k: 'ocean_agreeableness', l: 'Amabilidade'}, {k: 'ocean_neuroticism', l: 'Estabilidade'}].map(o => (
                   <div key={o.k} className="bg-slate-950 p-6 rounded-[35px] border border-white/5 text-center shadow-inner group hover:border-cyan-500/20 transition-all">
                      <p className="text-[9px] font-black uppercase text-slate-700 mb-3 tracking-widest">{o.l}</p>
              ...
              <button 
                onClick={() => { 
                  navigator.clipboard.writeText(`RELATÓRIO LION: ${viewReport.name}\nPerfil: ${viewReport.behavioral_profile}\nLocal: ${viewReport.cidade}/${viewReport.uf}\nFilhos: ${viewReport.has_children}`); 
                  alert("Relatório copiado!"); 
                }} 
                className="w-full bg-cyan-500 text-slate-950 py-12 rounded-[60px] font-black uppercase text-[15px] tracking-[0.8em] shadow-2xl flex items-center justify-center gap-8 italic hover:bg-white transition-all shadow-glow"
              >
                <Copy size={32}/> Copiar Dossiê Completo
              </button>
            </motion.div>
          </div>
        )}
      ...
      {/* MODAL ERP (CONTROLE FINANCEIRO) - MANTIDO INTEGRALMENTE */}
      <AnimatePresence>
        {actingContract && (
          <div className="fixed inset-0 z-[190] bg-slate-950/99 backdrop-blur-3xl flex items-center justify-center p-6 text-left">
             <motion.div initial={{ scale: 0.9, opacity: 0, y: 30 }} animate={{ scale: 1, opacity: 1, y: 0 }} className="bg-slate-900 border border-green-500/30 p-16 rounded-[80px] max-w-4xl w-full relative">
                ...
                <form className="space-y-10" onSubmit={handleFinalizeHire}>
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-3"><label className="text-[11px] font-black uppercase text-slate-700 ml-8 tracking-widest">Instituição Cliente</label><input name="client" required placeholder="Ex: BASA / Deskcorp" className="w-full bg-slate-950 border border-white/10 p-8 rounded-[35px] text-white font-bold uppercase outline-none focus:border-green-500 shadow-inner" /></div>
                   ...
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-3"><label className="text-[11px] font-black uppercase text-slate-700 ml-8 tracking-widest">Salário Bruto (R$)</label><input name="salary" type="number" required placeholder="10000" className="w-full bg-slate-950 border border-white/10 p-8 rounded-[35px] text-white font-bold outline-none focus:border-green-500 shadow-inner" /></div>
                      <div className="space-y-3"><label className="text-[11px] font-black uppercase text-slate-700 ml-8 tracking-widest">Início</label><input name="start" type="date" required className="w-full bg-slate-950 border border-white/10 p-8 rounded-[35px] text-white font-bold outline-none focus:border-green-500 shadow-inner" /></div>
                   </div>
