...
import { 
  ...
  UserCircle, Star, Target, Copy, CheckCircle2, ChevronRight, Settings, Database, Activity, Mail, 
  Smartphone, BadgeCheck, Link, Award, BarChart3, Download, FileCheck, Archive, ToggleLeft, 
  ToggleRight, Briefcase, Calendar, Info, Trash2, ShieldAlert, Fingerprint, RefreshCw, Layers, 
  TrendingUp, Heart, Glasses, BarChart, Check, Ban
} from 'lucide-react';
...
export default function AdminRH({ tickets, vacancies, onAddJob, onUpdateJob, onUpdateERP }: AdminProps) {
  ...
  const handleFinalizeHire = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    onUpdateERP(actingContract.id, {
      ...
      first_salary: salary,
      hiring_fee: calculateHiringFee(salary, actingContract.seniority || 'Junior'),
  ...
  return (
    <section className="container mx-auto px-6 py-12 text-left">
      <div className="bg-slate-900 p-12 md:p-20 rounded-[100px] border border-white/5 shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col xl:flex-row justify-between items-center mb-16 border-b border-white/5 pb-14 gap-12 relative z-10">
          <div className="flex flex-wrap bg-slate-950 p-3 rounded-[45px] border border-white/5 shadow-inner">
            ...
            <button onClick={() => setActiveTab('jobs_approval')} className={`px-12 py-6 rounded-[35px] text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'jobs_approval' ? 'bg-orange-500 text-white shadow-glow' : 'text-slate-500 hover:text-white'}`}>Vagas Pendentes</button>
            <button onClick={() => setActiveTab('management')} className={`px-12 py-6 rounded-[35px] text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'management' ? 'bg-purple-500 text-white shadow-glow' : 'text-slate-500 hover:text-white'}`}>Gestão ERP</button>
            <button onClick={() => setActiveTab('balance')} className={`px-12 py-6 rounded-[35px] text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'balance' ? 'bg-green-500 text-white shadow-glow' : 'text-slate-500 hover:text-white'}`}>Balanço</button>
          </div>
        ...
        {activeTab === 'jobs_approval' ? (
           <div className="overflow-x-auto relative z-10 animate-in fade-in duration-700 text-left">
              <table className="w-full">
                <thead><tr className="text-[11px] uppercase font-black text-slate-600 border-b border-white/5 pb-12 tracking-[0.5em]"><th className="pb-12">Solicitante</th><th className="pb-12">Título da Vaga</th><th className="pb-12 text-right">Decisão Auditoria</th></tr></thead>
                <tbody className="text-[13px]">{tickets.filter(t => t.role === 'client' && t.status === 'Pendente').map(v => (
                  <tr key={v.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-all group">
                    <td className="py-12"><div className="flex items-center gap-6"><Building2 size={24} className="text-orange-500"/><div className="flex flex-col"><span className="text-white font-black uppercase italic">{v.name}</span><span className="text-[9px] text-slate-600 font-mono">{v.id}</span></div></div></td>
                    <td className="py-12 text-slate-400 font-bold uppercase italic">{v.job_title} ({v.seniority})</td>
           ...
           <div className="space-y-10 relative z-10 animate-in fade-in duration-700 text-left">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                 <div className="bg-slate-950 p-12 rounded-[60px] border border-white/5 shadow-inner group hover:border-green-500/20 transition-all">
                    <p className="text-[10px] font-black uppercase text-slate-600 mb-4 tracking-[0.4em] italic flex items-center gap-3"><Wallet size={16} className="text-green-500"/> Faturamento (Hiring Fee)</p>
              ...
              <div className="overflow-x-auto bg-slate-950 p-10 rounded-[60px] border border-white/5">
                <table className="w-full text-left">
                  <thead><tr className="text-[10px] uppercase font-black text-slate-700 border-b border-white/5 pb-8 italic tracking-[0.5em]"><th className="pb-8">Talento</th><th className="pb-8">Cliente</th><th className="pb-8">Valor Recebido</th><th className="pb-8">Histórico</th></tr></thead>
                  <tbody className="text-[12px]">{tickets.filter(t => t.status === 'Contratado').map(t => (
                    <tr key={t.id} className="border-b border-white/5 hover:bg-white/[0.01] transition-all">
                      ...
                      <td className="py-10 text-slate-600 italic text-[11px]">{t.hiring_notes || 'Sem notas'}</td>
                    </tr>
                  ))}</tbody>
                </table>
              </div>
          ...
          <div className="overflow-x-auto relative z-10 text-left animate-in slide-in-from-left duration-700">
            <table className="w-full">
              ...
              <tbody className="text-[13px]">{tickets.filter(t => t.role === 'candidate' && t.status !== 'Contratado').map(t => (
                <tr key={t.id} className="border-b border-white/5 hover:bg-white/[0.02] transition-all group">
                  <td className="py-14"><div className="flex items-center gap-6"><div className="p-5 rounded-[25px] border border-white/5 bg-cyan-500/10 text-cyan-500 shadow-inner"><UserCheck size={28} /></div><div className="flex flex-col"><span className="text-white text-lg font-black uppercase italic tracking-tighter leading-none">{t.name}</span><span className="text-[10px] text-slate-600 font-mono mt-2 tracking-widest">{t.seniority} • {t.area}</span></div></div></td>
                  <td className="py-14 text-center"><span className="text-[10px] font-black uppercase text-purple-500 tracking-widest leading-none">{t.behavioral_profile || 'discovery'}</span><div className="w-32 bg-white/5 h-1 rounded-full mt-3 mx-auto shadow-inner"><div className="bg-cyan-500 h-full rounded-full shadow-glow" style={{ width: `${(Number(t.ocean_openness) || 1) * 10}%` }} /></div></td>
      ...
      <AnimatePresence>{actingContract && (
        <div className="fixed inset-0 z-[170] bg-slate-950/99 backdrop-blur-3xl flex items-start justify-center p-6 text-left py-24 overflow-y-auto text-left">
      ...
      <AnimatePresence>{viewReport && (
        <div className="fixed inset-0 z-[180] bg-slate-950/99 backdrop-blur-3xl flex items-center justify-center p-6 text-left overflow-y-auto py-24 text-left">
          <motion.div initial={{ scale: 0.9, opacity: 0, y: 50 }} animate={{ scale: 1, opacity: 1 }} className="bg-slate-900 border border-cyan-500/30 p-14 md:p-24 rounded-[120px] max-w-6xl w-full relative shadow-2xl overflow-hidden shadow-cyan-500/10">
            ...
            <div className="flex flex-col md:flex-row items-center gap-16 mb-20 border-b border-white/5 pb-20 text-center md:text-left"><div className="p-12 bg-cyan-500 rounded-[60px] text-slate-950 shadow-[0_0_60px_rgba(6,182,212,0.5)] scale-110 font-black"><UserCircle size={100} /></div><div className="space-y-6"><h3 className="text-7xl font-black uppercase italic text-white italic tracking-tighter leading-none">{viewReport.name}</h3><p className="text-cyan-500 text-[15px] font-black uppercase tracking-[0.6em] italic mt-4 flex items-center gap-3 justify-center md:justify-start"><ShieldCheck size={18}/> Dossiê Deep Analysis Lion Rising • {viewReport.id}</p></div></div>
            <div className="grid md:grid-cols-2 gap-16 mb-20 text-left">
               <div className="p-16 bg-white/5 rounded-[90px] border border-white/5 relative overflow-hidden group hover:border-cyan-500/30 transition-all duration-700">
                  <Star className="absolute -right-16 -top-16 opacity-5 text-cyan-500 scale-150 transition-transform duration-1000" size={300} /><p className="text-[15px] font-black uppercase text-slate-500 mb-12 flex items-center gap-5 tracking-[0.4em] italic font-black"><Star size={28} className="text-cyan-500"/> Expertise Técnica Senioridade: {viewReport.seniority}</p><p className="text-xl text-slate-300 leading-relaxed italic font-light tracking-wide">{viewReport.skills_summary}</p>
               </div>
               <div className="p-16 bg-white/5 rounded-[90px] border border-white/5 relative overflow-hidden group hover:border-purple-500/30 transition-all duration-700">
                  <Target className="absolute -right-16 -top-16 opacity-5 text-purple-500 scale-150 transition-transform duration-1000" size={300} /><p className="text-[15px] font-black uppercase text-slate-500 mb-12 flex items-center gap-5 tracking-[0.4em] italic font-black"><Target size={28} className="text-purple-500"/> Análise DISC & Pessoal</p><p className="text-lg text-slate-500 font-black uppercase tracking-widest mb-6">Filhos: {viewReport.has_children} • Civil: {viewReport.marital_status} • Hobby: {viewReport.hobbies}</p><p className="text-xl text-slate-300 leading-relaxed italic font-light tracking-wide">{viewReport.behavioral_summary}</p>
               </div>
            </div>
            ...
            <button onClick={() => { navigator.clipboard.writeText(`DOSSIÊ LION SOLUTION\nCandidato: ${viewReport.name}\nNível: ${viewReport.seniority}\nDISC: ${viewReport.behavioral_profile}\nFilhos: ${viewReport.has_children}`); alert("Dossiê de Alta Performance copiado com sucesso."); }} className="w-full bg-cyan-500 text-slate-950 py-12 rounded-[80px] font-black uppercase text-[18px] tracking-[0.8em] shadow-2xl flex items-center justify-center gap-8 italic hover:bg-white transition-all duration-1000 group shadow-cyan-500/30 shadow-glow"><Copy size={36} className="group-hover:rotate-12 transition-transform"/> Copiar Relatório Completo</button>
          </motion.div>
        </div>
      )}</AnimatePresence>
    </section>
